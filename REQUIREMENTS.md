...
3. Load the designated map file; the server may assume it is [valid](#valid-maps), but shall verify the file can be opened for reading.
...
7. Accept up to *MaxPlayers* players; if a player exits or quits the game, it can neither rejoin nor be replaced. Thus: once a player has quit, its letter is never re-used.
...
9. Send all client keystrokes to the server.
